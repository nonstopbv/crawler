...
public final class StringUtils {
	...
	/**
	 * Use regular expressions to extract the specified string in the content
	 * 
	 * @param content
	 * @param regex
	 * @return List the extracted string list
	 */
	public static List<String> regexExtractor(String content, String regex) {
		List<String> result = new ArrayList<String>();
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(content);
		while (matcher.find()) {
			for (int i = 1; i <= matcher.groupCount(); i++) {
				String matcherResult = matcher.group(i);
				if (isNotEmpty(matcherResult)) {
					result.add(matcherResult);
				}
			}
		}
		return result;
